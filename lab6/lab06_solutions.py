...
def do_ping():
    ...
    log_to_csv(
        LOG_FILE, "ping", host, ping_data["avg_ms"], ping_data["status"]
    )
...
def do_nslookup():
    ...
    log_to_csv(
        LOG_FILE, "nslookup", domain, dns_data["ip"], dns_data["status"]
    )
    print("Result logged.")
...
def do_network_info():
    ...
    try:
        ...
        log_to_csv(
            LOG_FILE, "ipconfig", "all",
            net_data["mac"] + " / " + net_data["ip"], "Captured"
        )
        print("Result logged.")
    except Exception as e:
        print("  Error: " + str(e))
...
def do_arp_table():
    ...
    try:
        ...
        log_to_csv(
            LOG_FILE, "arp", "local",
            str(len(devices)) + " devices", "Captured"
        )
        print("\nResult logged.")
    except Exception as e:
        print("  Error: " + str(e))
