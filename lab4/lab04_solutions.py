# Lab 04: Loops and Functions
# Student Name: Ben Morrison
...
def robot_returns_to_origin(moves):
    ...
    # Walk through each move and update position
    for move in moves:
        if move == "U":
            y += 1
        elif move == "D":
    ...
    # Back at spawn only if both coordinates are zero
    return x == 0 and y == 0
...
def two_sum_brute_force(numbers, target):
    # Check every pair (i, j) with j > i to avoid duplicates
...
def two_sum_optimized(numbers, target):
    # seen[value] = index; for each number, check if (target - number) was seen
    seen = {}
    for i in range(len(numbers)):
